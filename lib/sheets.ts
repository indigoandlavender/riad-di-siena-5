...
// OPS Sheet for guest management (separate from website content sheet)
const OPS_SHEET_ID = process.env.OPS_SHEET_ID || "1qBOHt08Y5_2dn1dmBdZjKJQR9ShjacZLdLJvsK787Qo";

// Master_Guests column headers - must match OPS dashboard exactly
const MASTER_GUESTS_HEADERS = [
  "booking_id", "source", "status", "first_name", "last_name", "email", "phone",
  "country", "language", "property", "room", "check_in", "check_out", "nights",
  "guests", "adults", "children", "total_eur", "city_tax", "special_requests",
  "arrival_time_stated", "arrival_request_sent", "arrival_confirmed", 
  "arrival_time_confirmed", "read_messages", "midstay_checkin", "notes",
  "created_at", "updated_at"
];

export async function appendToOpsSheet(booking: {
  bookingId: string;
  firstName: string;
  lastName: string;
  email: string;
  ...
  try {
    ...
    // Build row matching Master_Guests headers exactly
    const row = [
      ...
      String(booking.nights || ""), // nights
      String(booking.guests || ""), // guests
      String(booking.guests || ""), // adults (assume all adults)
      "0",                         // children
      String(booking.total || ""), // total_eur
      "",                          // city_tax
      booking.message || "",       // special_requests
      "",                          // arrival_time_stated
    ...
    console.error("Error appending to OPS Master_Guests:", error);
    return false;
  }
}
